# Filename : train.py
# Author   : Saravanan TK
# Course   : Udacity - Artificial Intelligence and Python Nanodegree
# Date     : 08-10-2018
...
'''
def eval_model(testloader, model, criterion, gpu):
...
'''
def train_model(model, trainloader, testloader, epochs, print_every, learning_rate, hidden_units, gpu, class_to_index, arch):   
    ''' Function is totally derived from Matt Leonard's PyTorch lessons
        Refer to Section 8 of PyTorch - Transfer learning. 
    '''
    ...
    for e in range(epochs):
        ...
        for ii, (inputs, labels) in enumerate(trainloader):
            ...
            if steps % print_every == 0: 
                model.eval() 
                eval_loss, eval_accuracy = eval_model(testloader, model, criterion, gpu)
...
def get_data_loaders(source_dir):
    ''' Function is totally derived from Matt Leonard's PyTorch lessons
        Refer to Section 1 of PyTorch - Transfer learning. 
    '''
...
def main():
    ...
    model, criterion, optimizer = train_model(
        model,
    ...
    loss, accuracy = eval_model(dataloaders['validation'], model, criterion, args["gpu"])
    print("Validation Data Loss {:.3f}".format(loss), "Validation Data Accuracy: {:.3f}".format(accuracy))
    loss, accuracy = eval_model(dataloaders['testing'], model, criterion, args["gpu"])
